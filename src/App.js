...
class App extends Component {
  ...
  removeCountry = countryId => {
    ...
    const filteredCountries = countriesList.map(eachCountry => {
      if (eachCountry.id === countryId) {
        const remove = {...eachCountry, isVisited: !eachCountry.isVisited}
        return remove
      }
      return eachCountry
    })
    const resultList = filteredCountries.filter(
  ...
  renderNoCountries = () => (
    <div className="no-countries-cont">
      <p className="name">No Countries visited Yet!</p>
    </div>
  )
  ...
  render() {
    ...
    const visitedCoutriesList = countriesList.filter(
    ...
    return (
      <div className="app-container">
        ...
        {visitedCoutriesList.length > 0 ? (
          <ul className="visited-countries-container">
            {visitedCoutriesList.map(eachCountry => (
              <VisitedCountry
                key={eachCountry.id}
